...
export const assessmentsApi = {
  ...
  createSession: (assessmentId: number, candidateName?: string, candidateId?: number, candidateEmail?: string) =>
    api.post<{ session: Session; invite_url: string }>(
      `/assessments/${assessmentId}/sessions`,
      { session: { candidate_name: candidateName, candidate_id: candidateId, candidate_email: candidateEmail } }
    ),
};
